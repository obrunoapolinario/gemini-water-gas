...
export const insertMeasureSchema = uploadSchema
	.extend({
		measure_value: z.coerce.number().min(0, "Measure value must be greater than 0"),
...
export const confirmSchema = z.object({
	...
	confirmed_value: z.coerce.number().min(0, "Confirmed value must be greater than 0"),
});
...
export const measureResponseSchema = z.object({
	measure_uuid: z.string().uuid("Invalid measure UUID"),
	measure_datetime: z.date().transform((date) => date.toISOString()),
	measure_type: measureTypeEnumSchema,
